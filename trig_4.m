...
% path(path,'E:\personal\Research files\MS Research Work\paper1\scenario4\video data set\parking\input');
%--------------------------------------------------------------------------
count_detect=0;
count_undetect=0;
count_detected=[];
...
%     f_g4_v=f_g4-(f_g4_m*ones(size(f_g4)));
    ...
    f_gass1=(1/sqrt(2*pi*var(nI_c1)))*exp(-(nI_c1-mean(nI_c1)).^2/var(nI_c1))-(1/sqrt(2*pi*var(nI_b1)))*exp(-(nI_b1-mean(nI_b1)).^2/var(nI_b1));
    f_gass2=(1/sqrt(2*pi*var(nI_c1)))*exp(-(nI_c2-mean(nI_c2)).^2/var(nI_c2))-(1/sqrt(2*pi*var(nI_b2)))*exp(-(nI_b2-mean(nI_b2)).^2/var(nI_b2));
    f_gass3=(1/sqrt(2*pi*var(nI_c1)))*exp(-(nI_c3-mean(nI_c3)).^2/var(nI_c3))-(1/sqrt(2*pi*var(nI_b3)))*exp(-(nI_b3-mean(nI_b3)).^2/var(nI_b3));
    f_gass4=(1/sqrt(2*pi*var(nI_c1)))*exp(-(nI_c4-mean(nI_c1)).^2/var(nI_c4))-(1/sqrt(2*pi*var(nI_b4)))*exp(-(nI_b4-mean(nI_b4)).^2/var(nI_b4));
    %----------------------------------------------------------------------
    %finding the covarience between background and current image
    f_g1_cov=corr(nI_b1,nI_c1); %computing the histogram covarience in the first block
    f_g2_cov=corr(nI_b2,nI_c2); %computing the histogram covarience in the second block
    f_g3_cov=corr(nI_b3,nI_c3); %computing the histogram covariance the 3rd block
    f_g4_cov=corr(nI_b4,nI_c4); %computing the histogram covariance in the 4th block
    
    
    %----------------------------------------------------------------------
    %finding Threshold values 
    T1=abs(sum(f_gass1)); %determining the first Threshold
    T2=abs(sum(f_gass2)); %determining the second Threshold
    T3=abs(sum(f_gass3)); %determining the 3rd Threshold
    T4=abs(sum(f_gass4)); %determining the 4th Threshold

...
rmpath 'E:\personal\Research files\MS Research Work\paper1\scenario4\video data set\pedestrians\input';
...
for i=1800:1:2000
    figure(1);
imshow(rec1,st,pr)
xlabel(cat(1,['Received Image Frame',num2str(i)]))
...
%     f_g4_v=var(f_g4);
    ...
    for j=1:length (T_t)
       ...
       I_foreground=medfilt2(I_foreground,[3,3]);
        ...
        if T_t(j)>0.02&&sum(I_foreground(:))>=6500
...
if j==1
 ...
 rec1(1:row_n/2,1:col_n/2)=I_c1+2*randn(1,1); %getting first 32 x 32 pixel image; %getting first 32 x 32 pixel image
elseif j==2

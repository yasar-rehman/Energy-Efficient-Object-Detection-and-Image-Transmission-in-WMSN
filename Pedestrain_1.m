...
% if j==1
% 
%  rec1(1:row_n/2,1:col_n/2)=I_c1+(I_c1*randn(1,1)/max(I_c1(:))); %getting first 32 x 32 pixel image; %getting first 32 x 32 pixel image
% elseif j==2
%     fa_detection=fa_detection+1;
%  rec1(1:row_n/2,(col_n/2)+1:col_n)= I_c2+(I_c2*randn(1,1)/max(I_c2(:))); %getting the second 32 x 32 image; %getting the second 32 x 32 image
%  tr_detection=tr_detection+1;
% elseif j==3
%  rec1(row_n/2+1:row_n,1:col_n/2)=I_c3+(I_c3*randn(1,1)/max(I_c3(:))); %getting the third 32 x 32 image; %getting the third 32 x 32 image
%  tr_detection=tr_detection+1;
% elseif j==4
%  rec1(row_n/2+1:row_n,col_n/2+1:col_n)=I_c4+(I_c4*randn(1,1)/max(I_c4(:)));
%  fa_detection=fa_detection+1;
% end
...
%     Ene_2a=((VB*I_cam_a*clk_c)+(I_mote_a_tof*VB));
%     Ene_ta=Ene_2a+Ene_1a;
%     T_energy_a=[T_energy_a,Ene_ta];
%     Ene_1a=Ene_ta;
 ...
 %Transmission using direct appraoch
    ...
    T_energy_aT=[T_energy_aT,Ene_1a];
    Ene_1=Ene_t;
end
%--------------------------------------------------------------------------
